...
export async function execute(_req, res, target) {
  ...
  const row = new ActionRowBuilder().addComponents(getWhatButton(target), getWhereButton(target))
...
function getHomeEmbed(item) {
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      {
        name: 'Where?',
        value: 'Find what locations give you the best\nchance at catching a new fish!'
      }
    )
    .setFooter(footer)
...
function getWhereButton(target) {
  return new ButtonBuilder().setCustomId(`fish-where:${target}`).setLabel('Where?').setStyle('Secondary')
}

function getWhatButton(target) {
  return new ButtonBuilder().setCustomId(`fish-what:${target}`).setLabel('What?').setStyle('Secondary')
}

function getHomeButton(target) {
  return new ButtonBuilder().setCustomId(`fish-home:${target}`).setLabel('Home').setStyle('Primary')
}

function getRefreshButton(type, target) {
  return new ButtonBuilder().setCustomId(`fish-refresh:${type}:${target}`).setLabel('Refresh').setStyle('Danger')
}

function getHomeRow(target) {
  return new ActionRowBuilder().addComponents(getWhatButton(target), getWhereButton(target))
}
...
function getFishInfo(toon, type) {
  ...
  if (type === 'what') {
    topFive = fishcalc
      .sortBestRarity()
      .filter(f => f.probability > 0 && f.buckets.confBuckets > 0) // Skip 0 values
      .slice(0, 5)
      .map(
        (f, index) =>
          `**${index + 1}. ${f.name} (${(f.probability * 100).toFixed(2)}%)**` +
          `Location: ${f.location}\n` +
          `Confident Buckets: ${f.buckets.confBuckets}\n` +
          `Average Buckets: ${f.buckets.avgBuckets}`
      )
      .join('\n\n') // Single line space between entries
