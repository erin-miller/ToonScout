...
function getHomeButton(target) {
  return new ButtonBuilder().setCustomId(`golf-home:${target}`).setLabel('Home').setStyle('Secondary')
}

function getAdviceButton(target) {
  return new ButtonBuilder().setCustomId(`golf-advice:${target}`).setLabel('Advice').setStyle('Secondary')
}

function getRefreshButton(type, target) {
  return new ButtonBuilder().setCustomId(`golf-refresh:${type}:${target}`).setLabel('Refresh').setStyle('Danger')
}
