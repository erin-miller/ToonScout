...
export async function execute(res) {
  ...
  const embed = new EmbedBuilder().setColor('DarkPurple').setTimestamp(new Date(meter.asOf * 1000))
  ...
  if (meter) {
    ...
    if (status === 'Active') {
      ...
      embed.addFields({ name: 'Team', value: teams, inline: true }, { name: 'Percentage', value: points, inline: true })
