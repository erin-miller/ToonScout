...
function getRow(target, id) {
  ...
  const filtered = buttons.filter(button => !button.data.custom_id.startsWith(`${ids.name}-${id}`))
...
function getUniteEmbed(item) {
  ...
  const fields = Object.entries(unites).flatMap(([, variants]) =>
...
function getSummonsEmbed(item) {
  ...
  const fields = Object.entries(summons).map(([, summon]) => ({
    name: summon.name,
    value:
...
function getHomeButton(target) {
  return new ButtonBuilder().setCustomId(`${ids.name}-${ids.home}:${target}`).setLabel('Home').setStyle('Primary')
}
...
function getSOSButton(target) {
  return new ButtonBuilder().setCustomId(`${ids.name}-${ids.sos}:${target}`).setLabel('SOS').setStyle('Secondary')
}

function getUniteButton(target) {
  return new ButtonBuilder().setCustomId(`${ids.name}-${ids.unite}:${target}`).setLabel('Unites').setStyle('Secondary')
}
