...
router.post('/get-reward-sums', async (req, res) => {
  ...
  try {
    const sumSos = rewards.sos ? Object.values(rewards.sos).reduce((sum, value) => sum + value, 0) : 0

    const sumUnites = rewards.unites
      ? Object.values(rewards.unites).reduce((sum, category) => {
          return sum + Object.values(category).reduce((categorySum, value) => categorySum + value, 0)
        }, 0)
      : 0

    const sumSummons = rewards.summons
      ? Object.values(rewards.summons).reduce((sum, summon) => {
          return sum + (summon.single ? 1 : 0) + (summon.building ? 1 : 0) + (summon.invasion ? 1 : 0)
        }, 0)
      : 0
    ...
    const totalRewards = sumSos + sumUnites + sumSummons + sumRemotes + (rewards.pinkslips || 0)
