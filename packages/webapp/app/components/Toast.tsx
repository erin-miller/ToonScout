...
const Toast: React.FC<ToastProps> = ({ message, show, onClose, duration = 4000, persistent = false, children }) => {
  React.useEffect(() => {
  ...
  const toastContent = (
    <div className="fixed bottom-8 right-8 z-50 bg-pink-700 text-white px-6 py-3 rounded-lg shadow-lg text-lg animate-fade-in flex items-center gap-2">
      {children}
      <span className="ml-2 font-semibold text-xl drop-shadow-sm">{message}</span>
      <button
