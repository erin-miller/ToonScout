...
import { Rewards, RewardSums } from '@/app/types'

const getRewardsHash = (rewards: Rewards) =>
...
const RewardsTab: React.FC<TabProps> = ({ toon }) => {
  ...
  const renderSOSSort = () => {
    const sortOptions = ['All', 'Restock', 'Toon-Up', 'Trap', 'Lure', 'Sound', 'Throw', 'Squirt', 'Drop', 'Other']
      ...
      <div className="tab-reward-container">
        ...
        <div className="reward-container">
          {rewardTypes.map(type => {
            ...
            const secondaryImage = rewardImages[type.toLowerCase() as keyof typeof rewardImages]
            ...
            return (
              ...
              >
                <>
                  <Image src={getButtonImage()} className="base-image" alt={type + ' base'} width={112} height={112} />
                  <Image
                    src={secondaryImage}
                  ...
                  {showCount && countValue !== null && (
                    <div className="text-lg lg:text-2xl 2xl:text-3xl justify-end items-end">
                      <span className="absolute bottom-1.5 right-2.5 text-blue-950 ">{countValue}</span>
                      <span className="absolute bottom-2 right-3 text-gray-100">{countValue}</span>
                    </div>
                  )}
                </>
              </button>
