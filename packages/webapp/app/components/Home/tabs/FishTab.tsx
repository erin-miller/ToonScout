...
import { sumFish } from '../../../utils/tabUtils'
...
const FishTab: React.FC<TabProps> = ({ toon }) => {
  ...
  window.addEventListener('fishChange', _event => {
    const storedFishDisplay = parseInt(localStorage.getItem('fishDisplayType') || '1')
    if (storedFishDisplay !== displayType) {
      setDisplayType(storedFishDisplay as 1 | 2)
    }

    const storedBucketType = parseInt(localStorage.getItem('bucketType') || '1')
    if (storedBucketType !== bucketType) {
      setBucketType(storedBucketType as 1 | 2)
    }

    const storedShowCaught = JSON.parse(localStorage.getItem('showCaught') || 'false')
    if (storedShowCaught !== showCaught) {
      setShowCaught(storedShowCaught)
    }

    const storedShowTime = JSON.parse(localStorage.getItem('showTime') || 'false')
  ...
  return (
    <>
        ...
        <div className="fish-container">
          ...
          <div className="fish-table relative">
            <div className="flex text-xl md:text-2xl xl:text-3xl pb-2 ml-5 mr-9 space-x-2">
              {displayType == 1 && <div className="fish-table-header w-2/3">Fish</div>}
              ...
              <div className={`fish-table-header w-1/3 ${showTime ? 'opacity-100' : 'opacity-0'}`}>Time</div>
              <button
                    ...
                    return (
                      <div className="fish" key={index}>
                        ...
                        <div className="fish-info w-2/3 text-left">
                          <span>
                            {(item.probability * 100).toFixed(2)}% or {getBuckets(item.buckets)} buckets
                          </span>
                        </div>
                        <div className={`fish-info w-1/3 text-left ${showTime ? 'opacity-100' : 'opacity-0'}`}>
              ...
              {displayType == 2 &&
                (locations && !isLocEmpty() ? (
                  locations.map((location, index) => {
                    ...
                    return (
                      <div className="fish" key={index}>
                        <div className="fish-info w-2/3 text-left">{location[0]}</div>

                        <div className="fish-info w-2/3 text-left">
                          <span>
                            {(total * 100).toFixed(2)}% or {getBuckets(buckets)} buckets
                          </span>
                        </div>
      ...
      {showSettingsModal && (
        <ToonSettingsModal toon={toon} index={activeIndex} isOpen={showSettingsModal} onClose={toggleSettingsModal} />
      )}
    </>
  )
}
