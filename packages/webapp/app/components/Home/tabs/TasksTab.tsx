...
import {
  applyOverrideToMatch,
  getTaskSignatureParts,
...
const TasksTab: React.FC<TabProps> = ({ toon: toons }) => {
  const [tasklineMatches, setTasklineMatches] = useState<
    Map<string, TaskMatch | null>
  >(new Map());
  ...
  const previousStepsRef = useRef<
    Map<string, { stepNumber: number; tasklineId: string }>
  >(new Map());
  ...
  const tasksSignature = useMemo(() => {
    return [
      `timestamp:${toons.timestamp}`,
      toonTasks.map((task, i) => createTaskMatchKey(task, i)).join("|"),
    ].join("|");
  ...
  const computedMatches = useMemo(() => {
    ...
    toonTasks.forEach((task, index) => {
      ...
      if (match) {
        ...
        const overriddenStepNumber = applyOverrideToMatch(
          task,
          match.step.order,
          match.taskline.id,
          toonId
        );
        const overriddenStep = match.taskline.steps.find(
          (s) => s.order === overriddenStepNumber
        );
  ...
  useEffect(() => {
    ...
    tasklineStepCandidates.forEach(({ stepNumber: currentStepNumber, task }, tasklineId) => {
      const previousStepNumber =
        previousTasklineStepsRef.current.get(tasklineId);
      if (previousStepNumber === undefined) {
        updateCurrentStep(tasklineId, currentStepNumber, false, toonId);
        // Update chain context: store where this step sends the toon
        updateChainContext(tasklineId, currentStepNumber, {
  ...
  useEffect(() => {
    ...
    const normalizeReward = (value?: string | null) =>
      (value ?? "").toLowerCase().trim();
    const candidateIndexes = [
      matchKeyIndex,
      selectedTaskMatch.taskIndex,
    ...
    const isSameTask = (candidate: ReturnType<typeof getCandidate>) => {
      ...
      const tasklineMatchesSelected =
        candidate.match.taskline.id === selectedTaskMatch.match.taskline.id;
      if (!tasklineMatchesSelected) return false;
      return (
        normalizeReward(candidate.task.reward) ===
        normalizeReward(selectedTaskMatch.task.reward)
      );
    };
    let candidate = candidateIndexes
      .map(getCandidate)
      .find((value) => isSameTask(value));
    ...
    const hasOverride =
      !!override && override.tasklineId === freshMatch.taskline.id;
    ...
    const shouldRefreshSelection =
      taskRefChanged || matchRefChanged || matchKeyChanged || taskIndexChanged;

    if (hasOverride) {
      const tasklineIdChanged =
        selectedTaskMatch.match.taskline.id !== freshMatch.taskline.id;
      if (
        !tasklineIdChanged &&
