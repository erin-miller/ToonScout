...
const InvasionsTab: React.FC<TabProps> = ({ toon }) => {
  ...
  const prevTimeLeftRef = React.useRef<Record<string, number | null | undefined>>({})
  ...
  const debouncedInvasions = useMemo(() => {
    return invasions.map(inv => {
      ...
      if (typeof prev === 'number' && typeof curr === 'number' && curr > prev && curr - prev < 120) {
  ...
  const relevantInvasions = useMemo(() => {
    ...
    return getRelevantInvasionsForTasks(toon.data.data.tasks, debouncedInvasions)
  }, [toon, debouncedInvasions])

  const sortedInvasions = useMemo(() => {
    if (!relevantInvasions.length) return debouncedInvasions
    const relevantKeys = new Set(relevantInvasions.map(inv => `${inv.cog}|${inv.district}`))
    const relevant = debouncedInvasions.filter(inv => relevantKeys.has(`${inv.cog}|${inv.district}`))
    const others = debouncedInvasions.filter(inv => !relevantKeys.has(`${inv.cog}|${inv.district}`))
    return [...relevant, ...others]
  }, [debouncedInvasions, relevantInvasions])

  return (
    <AnimatedTabContent>
      <div className="grid 2xl:grid-cols-2 gap-2 max-h-[60vh] overflow-y-auto pr-2 fish-scrollbar">
        {loading ? (
          <div className="flex items-center justify-center py-8">
            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-pink-500"></div>
            <span className="ml-3 text-gray-600 dark:text-gray-400">Loading invasions...</span>
          </div>
          ...
          <AnimatePresence initial={false}>
            {sortedInvasions.map(invasion => {
              ...
              const isRelevant = relevantInvasions.some(
                rel => rel.cog === invasion.cog && rel.district === invasion.district
              )
              return (
                <InvasionCard
