...
const GardenTab: React.FC<TabProps> = ({ toon }) => {
  ...
  const [_can, setCan] = React.useState('Bronze Can')
  const [currCombo, setCurrCombo] = React.useState(1)
  ...
  window.addEventListener('flowerChange', _event => {
    const storedFlowerType = parseInt(localStorage.getItem('flowerType') || '1')
  ...
  const removeDupes = (arr: FlowerResponse[]) => {
    return arr.filter(flower => !progress.some(progFlower => progFlower.name === flower.name))
  }
          ...
          <div className="flex w-full h-full bg-violet-50 dark:bg-gray-1000 shadow-sm p-2">
            ...
            <div className="flex flex-col items-start justify-start w-full h-full ml-2">
              ...
              <div className="flex flex-row w-full gap-1">
                {flowerCombo.map((combo: string, idx: number) => {
                  return (
                    <div key={idx} className="flex flex-row gap-0.5">
                      <Image
        ...
        <>
          ...
          <div className="flex flex-col h-full px-1">
            <div className="flex text-xl md:text-2xl xl:text-3xl pb-2 justify-between">
              <div className="flex items-center h-full">{currCombo} bean flowers</div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 2xl:grid-cols-3 gap-2">
            ...
            {flowerType >= 2 && renderFlowers(removeDupes(plantable), 'border-pink-500')}
