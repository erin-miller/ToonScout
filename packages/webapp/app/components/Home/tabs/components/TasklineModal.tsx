...
const TasklineModal: React.FC<TasklineModalProps> = ({
  ...
  const [selectedStepNumber, setSelectedStepNumber] = useState<number>(forceStepOrder ?? currentStep?.order ?? 1)
  ...
  const isProgressComplete = useMemo(() => {
    ...
    if (typeof progressData.text === 'string' && progressData.text.toLowerCase().includes('complete')) {
      return true
    }
  ...
  const effectiveStepNumber = useMemo(() => {
    ...
    return selectedStepNumber
  }, [currentStep, forceStepOrder, hasUserOverride, isProgressComplete, selectedStepNumber, totalSteps])

  const completionPercent = totalSteps > 0 ? Math.round((effectiveStepNumber / totalSteps) * 100) : 0

  const fullObjectiveText = useMemo(() => buildTaskObjectiveText(task), [task])

  const taskStreet = useMemo(() => getTaskStreet(task), [task])
  const selectedStep = useMemo(
    () => taskline.steps.find(s => s.order === effectiveStepNumber) ?? taskline.steps[0],
  ...
  }, [currentStep?.order, forceStepOrder, minStepFromProgress, task, taskline.id, toonId, progress?.lastSeenStep])
            ...
            >
              {modalView === 'steps' ? (
                <div className="bg-gradient-to-r from-pink-900 to-pink-700 dark:from-pink-800 dark:to-pink-900 text-white px-6 pt-6 pb-3 relative flex-shrink-0">
                  ...
                  <h2 className="text-xl sm:text-2xl font-bold pr-10">{task.reward}</h2>
