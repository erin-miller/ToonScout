...
import { findActiveOptionForTask, getStepOptionDisplay, formatOptionLabel } from '@/app/utils/tasklineDisplayUtils'
...
export const TasklineStepList: React.FC<TasklineStepListProps> = ({
  ...
  const renderStepObjective = (step: TasklineStep) => {
    ...
    if (parsed && parsed.options.length > 0) {
      const matchedOption =
        ...
        step.order === currentStep.order
          ...
          : null
      const activeOption = matchedOption ?? findActiveOptionForTask(step, fullObjectiveText, task)

      return (
        <div className="flex-1">
          <p className="font-semibold text-gray-800 dark:text-gray-100">{parsed.heading}</p>
                ...
                >
                  <span
                    className={`mt-1.5 w-1.5 h-1.5 rounded-full flex-shrink-0 ${
                      isActive ? 'bg-emerald-500 dark:bg-emerald-300' : 'bg-gray-400 dark:bg-gray-500'
                    }`}
                  />
                  <span className={isActive ? 'font-semibold flex-1' : 'flex-1'}>{label}</span>
                </li>
              )
            })}
          </ul>
    ...
    return <p className="font-semibold text-gray-800 dark:text-gray-100">{step.objective}</p>
  }
            ...
            >
              ...
              >
                <div className="flex gap-3">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">{renderStepObjective(step)}</div>
                    ...
                    {step.details && step.details.length > 0 && (
                      <div className="mt-2">
                        <p className="text-sm font-medium text-gray-700 dark:text-gray-200">Details:</p>
                        <ul className="mt-1 space-y-1">
                          {step.details.map((detail, idx) => (
                            <li key={idx} className="text-sm flex items-start gap-2 text-gray-700 dark:text-gray-200">
