...
import { getRelevantInvasionsForTasks, sanitizeCogName, getCogImage } from '@/app/utils/invasionUtils'
import InvasionToast from '@/app/components/InvasionToast'
import type { StaticImageData } from 'next/image'
import { playNotificationSound, showNativeNotification } from '@/app/utils/notificationUtils'
...
export function useInvasionNotifications({
  ...
  const [cogIcon, setCogIcon] = useState<StaticImageData | undefined>(undefined)
  ...
  useEffect(() => {
    const handleInvasionNotification = (event: CustomEvent) => {
      ...
      const { message, invasions: eventInvasions, showToast: eventShowToast, playSound } = event.detail
      let cogs: string[] = []
      if (Array.isArray(eventInvasions) && eventInvasions.length > 0) {
        cogs = eventInvasions.map(i => sanitizeCogName(i.cog))
      }
    ...
    window.addEventListener('invasionNotification', handleInvasionNotification as EventListener)
    return () => window.removeEventListener('invasionNotification', handleInvasionNotification as EventListener)
  }, [notificationsEnabled, handleNotification, soundRepeat, soundRepeatInterval, nativeNotifEnabled])
  ...
  useEffect(() => {
    ...
    const relevantKeys = relevant.map(i => `${sanitizeCogName(i.cog)}|${i.district}`)
    ...
    const newKeys = relevantKeys.filter(
      key => !prevRelevantKeys.current.includes(key) && !dismissedCogs.includes(key.split('|')[0])
    )
    ...
    const goneKeys = prevRelevantKeys.current.filter(key => !relevantKeys.includes(key))
    if (goneKeys.length > 0 || relevantKeys.length === 0) {
      setShowToast(false)
      clearSoundInterval()
      setDismissedCogs(prev => prev.filter(cog => !goneKeys.some(key => key.startsWith(cog + '|'))))
    }
