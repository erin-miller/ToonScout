...
const SuitTab: React.FC<TabProps> = ({ toon }) => {
  ...
  useEffect(() => {
    ...
    if (!currAvail || !toon.data.data.cogsuits[dept]?.hasDisguise) {
      const firstAvail = avail.find(department => toon.data.data.cogsuits[department]?.hasDisguise)
      if (firstAvail) {
        setDept(firstAvail)
      }
    }
    ...
    <AnimatedTabContent>
      <div className="tab-suit-container">
        <div className="dept-container">
          ...
          >
            <Image src={imageAssets.emblem_sell} className="dept-photo" alt="Sellbot" width={96} height={96} />
          </button>
          <button
          ...
          >
            <Image src={imageAssets.emblem_cash} className="dept-photo" alt="Cashbot" width={96} height={96} />
          </button>
          <button
          ...
          >
            <Image src={imageAssets.emblem_law} className="dept-photo" alt="Lawbot" width={96} height={96} />
          </button>
          <button
          ...
          >
            <Image src={imageAssets.emblem_boss} className="dept-photo" alt="Bossbot" width={96} height={96} />
          </button>
        </div>
        <div className="suit-container">
          <div className="suit-overview">
            <div className="w-1/2  text-left">{getSuitName(toon, dept)}</div>
            <div className="w-1/5  text-left text-nowrap">{suit ? `Level ${suit.level}` : 'No suit available'}</div>
          ...
          <div className="promo-rec">
            ...
            <div className="footer pb-2 text-md sm:text-xl lg:text-2xl text-center">
              <div>{suit?.promotion?.current < suit?.promotion?.target && `Estimated Total: ${promo?.total}`}</div>
