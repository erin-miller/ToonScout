...
const ActivePortsContext = createContext<ActivePortsContext | undefined>(undefined)

export const ActivePortsProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  return (
    <ActivePortsContext.Provider value={{ activePorts, addPort, removePort }}>{children}</ActivePortsContext.Provider>
  )
}
...
export const useActivePortsContext = () => {
  ...
  if (!context) {
    throw new Error('useActivePortsContext must be used within an ActivePortsProvider')
  }
  return context
}
