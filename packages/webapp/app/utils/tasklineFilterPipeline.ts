...
type PipelineInput = {
  ...
  inspectStage?: (
    ...
    remaining: FilterCandidate[],
    previous: FilterCandidate[]
  ) => void;
};
...
export function runTasklineFilterPipeline({
  ...
  const eligibleCandidates = allowCandidate
    ? candidates.filter(allowCandidate)
    : candidates;
  ...
  // Stage 2: NPC taskline-level filter
  {
    ...
    const next = disabled.has("npc_taskline")
      ? filtered
      : filterByNpcTaskline(filtered, toNpc);
  ...
  {
    const next = disabled.has("reward")
      ? filtered
      : filterByReward(filtered, task.reward);
  ...
  // Stage 5: Numeric filter
  {
    const next = disabled.has("numeric")
      ...
      : filtered.filter(
          (candidate) =>
            !isNumericMismatch(task, objectiveText, candidate.objective.text)
        );
...
function filterByReward(
  candidates: FilterCandidate[],
  taskReward: string | null | undefined
): FilterCandidate[] {
  ...
  if (isTeleportAccessReward(taskReward)) {
    return filterWithFallback(candidates, (c) =>
      getCandidateReward(c).toLowerCase().includes("teleport access")
    );
  }
 ...
 */
function isNumericMismatch(
  task: Task,
  taskObjectiveText: string,
  ...
  const countMismatch =
    taskCount !== null && stepCount !== null && taskCount !== stepCount;
  const levelMismatch = !requirementsMatch(
    taskRequirements.level,
    stepRequirements.level
  );
  const storyMismatch = !requirementsMatch(
    taskRequirements.story,
    stepRequirements.story
  );

  return countMismatch || levelMismatch || storyMismatch;
}

function getTaskTargetCount(
  task: Task,
  requirements: ReturnType<typeof parseNumericRequirements>
): number | null {
