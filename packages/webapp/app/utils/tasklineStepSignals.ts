...
import { FilterCandidate, MatchQuality, ObjectiveMatchQuality } from './tasklineMatchingTypes'
...
function getFromMatch(task: Task, candidate: FilterCandidate): boolean {
  ...
  return (
    normalizedFrom === normalizedPrevDest ||
...
function calculateSignals(
  ...
  const locationMatch =
    taskLocation && candidate.objective.location ? locationsMatch(taskLocation, candidate.objective.location) : false
