...
import { CogData, InvasionData, Task } from '@/app/types'
...
export function getRelevantInvasionsForTasks(tasks: Task[], invasions: InvasionData[]) {
  ...
  return invasions.filter(invasion => {
    ...
    return incompleteTasks.some(task => {
      ...
      return Object.keys(cogNameMap).some(variant => {
        return cogNameMap[variant] === invasionKey && normalize(objText).includes(variant)
      })
    })
  })
}
...
function getCogNameVariants(cog: CogData) {
...
export function getCogImage(cogName: string): StaticImageData | undefined {
  ...
  const cog = cogsData.find((c: CogData) => c.name === canonical || c.fullname === canonical)
