...
export function getTasklineProgress(
  tasklineId: string,
  toonId?: string | null
): TasklineProgress | null {
...
export function updateCurrentStep(
  ...
  const nextStepNumber = allowRegression
    ? currentStepNumber
    : Math.max(currentStepNumber, progress.lastSeenStep);
 ...
 */
export function removeTasklineProgress(
  tasklineId: string,
  toonId?: string | null
): void {
  if (!toonId) return;
