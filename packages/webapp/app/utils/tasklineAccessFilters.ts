...
function getNormalizedPlaygroundCandidatesByTaskLaff(taskLaffFromTasks: number): string[] {
  return getPlaygroundByTaskLaffCandidates(taskLaffFromTasks).map((playground: string) => normalizePlace(playground))
}
...
export function getAllowedPlaygrounds(task: Task, toonData?: ToonData, overjoyed?: boolean): Set<string> | null {
  ...
  const gagCandidates = getPossiblePlaygroundsByGagTracks(gagTrackCount).map(value => normalizePlace(value))
  const laffCandidates = getNormalizedPlaygroundCandidatesByTaskLaff(estimation.taskLaffFromTasks)
  const canAccessStf = gagCandidates.includes('sellbot task force') && canAccessSellbotTaskForce(toonData)
  ...
  if (laffCandidates.length > 0) {
    const overlap = gagCandidates.filter(candidate => laffCandidates.includes(candidate))
    if (overlap.length > 0) {
      allowed = overlap
    }
  }
