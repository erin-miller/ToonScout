...
import { TaskCategory, MatchDebugInfo, TasklineFilterPipeline, TasklineFilterStageName } from './tasklineMatchingTypes'
 ...
 */
function resolveProgressTiebreaker(topGroup: MatchCandidate[], toonId: string | null): MatchCandidate {
  ...
  const stepNumbers = Array.from(new Set(similarCandidates.map(candidate => candidate.step.order))).sort(
    (a, b) => a - b
  )

  const preferredStep = findNextUncompletedStep(topCandidate.taskline.id, stepNumbers, toonId)

  if (preferredStep !== null) {
    const preferredCandidate = similarCandidates.find(candidate => candidate.step.order === preferredStep)
 ...
 */
function buildMatchResult(best: MatchCandidate, category: TaskCategory): { match: TaskMatch; debug: MatchDebugInfo } {
  const confidence = getConfidence(best.quality)
  const usedDeterministic = best.signals.npcMatch && (best.signals.buildingMatch || best.signals.locationMatch)
...
export function getTasklineFilterPipeline(
  ...
  const { stages, totalCandidates, candidates } = runTasklineFilterPipeline({
    ...
    allowCandidate: c =>
      c.taskline.category !== 'cog_disguise' || canConsiderCogDisguiseTaskline(c.taskline.id, toonData)
  })
  return {
    totalCandidates,
    stages,
...
export function findTasklineMatchWithToonDataDetailed(
  ...
  if (matchCandidates.length === 0) {
    ...
    const unmatchedReason = isGenericHqOfficerProfile(task)
      ? 'random_reward'
      : candidates.length
        ? 'no_match'
        : 'no_candidates'
...
export function findTasklineMatchWithToonData(task: Task, toonData?: ToonData, overjoyed?: boolean): TaskMatch | null {
