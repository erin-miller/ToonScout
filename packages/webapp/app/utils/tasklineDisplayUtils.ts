...
export const getStepOptionDisplay = (
  step: TasklineStep
): { heading: string; options: string[] } | null => {
  if (!step.options || step.options.length === 0) {
    return null;
  }
  const heading =
    step.objective.replace(/:\s*$/, "").trim() || "One of the following";
  return {
    heading,
...
export const findActiveOption = (
  ...
  const optionContexts = step.options
    .map(getContextTokens)
    .filter((contextTokens) => contextTokens.size > 0);
  const requiresContext =
    optionContexts.length > 0 &&
    optionContexts.some((contextTokens) => isTokenSubset(contextTokens, taskTokens));

  const matchingOptions = step.options.filter((option) =>
    tokensMatch(taskTokens, tokenize(option.objective))
  );
  ...
  if (requiresContext) {
    const contextualMatches = matchingOptions.filter((option) =>
      isTokenSubset(getContextTokens(option), taskTokens)
    );
  ...
  const contextFreeMatches = matchingOptions.filter(
    (option) => !option.location && !option.building
  );
...
export const findActiveOptionForTask = (
  ...
  for (const option of step.options) {
    if (option.building) {
      ...
      const buildingMatch = taskBuildings.some(
        (building) => building.toLowerCase().trim() === optionBuilding
      );
      if (buildingMatch) {
        return option;
      }
