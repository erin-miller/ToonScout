...
export const displaySuit = (toon: StoredToonData, type: string) => {
  ...
  const dept = toon.data.data.cogsuits[type]
  return (
...
export const sumGolf = (toon: StoredToonData) => {
  ...
  for (const trophy of golf_trophies) {
    const earned = toon.data.data.golf.find(item => item.name == trophy.description)?.num || 0
...
export const sumRace = (toon: StoredToonData) => {
  ...
  for (const trophy of race_trophies) {
    const earned = toon.data.data.racing.find(item => item.name == trophy.description)?.num || 0
    for (const val of trophy.values) {
      if (earned >= val) {
        count += 1
      }
...
export const sumFlowers = (toon: StoredToonData) => {
  const collection = toon.data.data.flowers.collection
  let count = 0
