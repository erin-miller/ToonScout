...
function mergeWithExisting(taskline, existing, { preferExisting }) {
  ...
  const mergedSteps = taskline.steps.map(step => {
    ...
    return {
      ...
      details: step.details ?? existingStep.details,
...
/**
 * Ensures all tasklines have unique IDs by appending _1, _2, etc. for duplicates
 */
function deduplicateIds(tasklines) {
  const idCounts = new Map();
  return tasklines.map(taskline => {
    const baseId = taskline.id;
    ...
    return { ...taskline, id: `${baseId}_${count}` };
  });
}
...
async function writeTasklines(fileName, tasklines, { preferExisting }) {
  ...
  // Deduplicate IDs before ordering
  const deduplicated = deduplicateIds(merged);
  const ordered = orderByExisting(deduplicated, existing);
