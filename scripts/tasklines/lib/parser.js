 ...
 * @returns {Array<{ order: number, objective: string, building?: string, location?: string, options?: Array<{ objective: string, building?: string, location?: string }>, details?: string[] }>}
 */
function parseToonTaskSteps(sectionText) {
  ...
  const parseOption = item => {
    const details = parseObjectiveDetails(item, { keepParenthetical: false });
    return {
  ...
  const parseInlineOptions = text => {
    const cleaned = stripWikiMarkup(text).trim();
    if (!cleaned) {
      return [];
    }
    const colonIndex = cleaned.indexOf(':');
    const content = colonIndex >= 0 ? cleaned.slice(colonIndex + 1).trim() : cleaned;
    if (!content) {
      return [];
    }
    ...
    if (semiSplit.length > 1) {
      return semiSplit.map(parseOption);
    }
    return [];
  };

  return rawSteps.map((step, index) => {
    const headingRaw = stripWikiMarkup(step.text).trim();
    if (step.subitems.length > 0) {
      const heading = headingRaw.replace(/:\s*$/, '').trim() || 'One of the following';
      const isOptionList = /:\s*$/.test(step.text.trim());
      if (isOptionList) {
        return {
          order: index + 1,
          objective: heading,
          options: step.subitems.map(parseOption),
        };
      }
      const details = step.subitems
        .map(item => stripWikiMarkup(item).trim())
        .filter(Boolean);
      return {
        order: index + 1,
        objective: heading,
    ...
    if (headingRaw) {
      const heading = headingRaw.replace(/:\s*$/, '').trim() || 'One of the following';
      const options = parseInlineOptions(step.text);
      if (options.length > 0) {
        return {
          order: index + 1,
          objective: heading,
